...
function fetchPosts() {
    return new Promise((res, req)=> {
        (setTimeout(() => res('Posts data'), 1000));
    });
}
...
function fetchFailure() {
    return new Promise((res, rej) => 
        setTimeout(() => {rej('Error occurred')}, 1000));
    
}

async function handlePromises() {
  try {
      const[sucess, failure] = await Promise
      .all([fetchSuccess(), fetchFailure()]);
  } catch (error) {
    console.log(error);
  }
}
...
function fetchWithTimeout(promise, timeout) {
    const timeOutPromise = new Promise((res, rej) => 
        setTimeout(() => rej('Timeout exceeded'), timeout)
    );
